...
def tc_ood_01():
    ...
    ood_imgs = [
        (np.random.rand(C.STD_IMG, C.STD_IMG, 3) * 255).astype(np.uint8)
    ...
    ood_rate = measured["ood_confident_detection_rate"]
    H.save_json(measured, C.RESULTS / "TC-OOD-01" / "ood.json")
    H.record(
        "TC-OOD-01",
        "OOD Input Rejection",
        "PASS"
        if (auroc >= C.OOD_MIN_AUROC and ood_rate <= C.OOD_MAX_CONFIDENT_RATE)
        else "MEASURED",
        f"PASS if AUROC >= {C.OOD_MIN_AUROC} (TBC) and confident-detection rate on OOD inputs "
        f"<= {C.OOD_MAX_CONFIDENT_RATE} (TBC); report AUROC and FPR@95%TPR.",
        measured,
        "Black-box OOD via output confidence; post-hoc scorers (ODIN/energy) can refine the score.",
        ["TC-OOD-01/ood.json"],
    )
    return measured
...
def run_all():
    print("[out-of-distribution verification]")
    tc_ood_01()
